...
from hw.dv.ahb3.ahb3_item import Ahb3LiteItem
...
class UartAhbBaseSequence(uvm_sequence):
    ...
    async def configure_uart(self, clk_div: int = 0):
        ...
        if (rd.rdata & 0x03FF0000) != (ctrl & 0x03FF0000):
            raise AssertionError("CTRL clk_div readback mismatch")
        if (rd.rdata & 0x0F) != (ctrl & 0x0F):
            raise AssertionError("CTRL enable bits readback mismatch")
    ...
    async def wait_for_status(self, mask: int, value: int, max_reads: int = 200):
        for _ in range(max_reads):
            ...
            if (rd.rdata & mask) == value:
                return rd.rdata
        raise AssertionError(f"STATUS mask 0x{mask:08x} did not reach value 0x{value:08x}")
