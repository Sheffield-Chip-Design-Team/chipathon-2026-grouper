...
# FIXEME - this should be split into multiple tests so that we can reproduce one failure at a time.
@cocotb.test()
async def test_ahb_spi_master(dut):
    ...
    # using the pre-existing functions means that we can check HRESP
