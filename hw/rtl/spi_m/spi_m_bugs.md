...
decoded MOSI stream by `hw/tb/spi_m/test_spi_m.py` (25 tests, all passing).
...
| `SPIM-ISSUE-030` | **A multi-lane `DATA` store did not stall the bus.** The comment on the multi-byte push claimed "the AHB transfer is stalled with `HREADYOUT` until they drain", but `HREADYOUT` was `~err_req` — only the error response ever pulled it low. The lanes were instead latched and drained in the background, which put the right bytes on the wire but left the described flow control unimplemented, and dropped lanes on a full FIFO with no back-pressure. | `push_stall` now holds `HREADYOUT` low while more than one lane is outstanding, and the AHB pipeline register is frozen for the duration so the next transfer's address phase cannot overwrite a stalled data phase. Specified as `GRPR-SPIM-024`; covered by `test_word_push_wait_states` and `test_byte_push_is_zero_wait`. |
| `SPIM-ISSUE-031` | **First attempt at that stall broke 8 of 23 tests** — same root cause as `SPIM-ISSUE-027`, one level along. `push_stall` was written as `(\|push_pending) && !tx_full`, but `push_pending` is a *register*: on the cycle the store lands it is still zero, so no stall was asserted then, and `HREADYOUT` instead went low one cycle later — against the *following* transfer. Since a push is always followed by the CMD write, it swallowed CMD and the transfer never started (empty MOSI stream, `BUSY` never clearing). | The stall is computed from `push_next`, a combinational view of the lanes outstanding *after* this cycle retires, so it asserts in the same cycle the store lands. The lesson from `-027` restated: anything qualifying an AHB data phase must be combinational in that phase, never a register sampled from it. |
