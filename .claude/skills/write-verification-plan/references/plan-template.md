...
**DUT:** `hw/rtl/<area>/<block>.sv` (and its AHB wrapper, e.g. `ahb_<block>`, when it exists)
...
<Exact environment assumptions and runnable `fusesoc run --no-export <target>` commands
against the block's core (cross-check `.github/sim-ci-targets.yaml`). Note any `fail_ok`
legs. Include formal invocations if any.>
