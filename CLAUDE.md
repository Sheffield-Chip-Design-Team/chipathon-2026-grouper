...
- **The `DRY_RUN` define** (set in `librelane/classic/config.yaml`'s `VERILOG_DEFINES`, alongside `MACRO_RAM` and `ROM_INIT_CONST`) builds a PnR-only variant of the SoC: `ahb_rom.sv` shrinks to 64 bytes holding the `sw/dry_run/rom_boot.S` stub that jumps to the RAM window on reset, and `ahb_spi_s` is replaced by `ahb_stub_slave`. The RAM is *not* stubbed — `MACRO_RAM` builds the real SRAM macros. It exists to characterize area/congestion/timing at 1100×1500 — it is not functional, and nothing in simulation should ever define it. There is no separate dry-run config any more: `dry_run_config.yaml`, `dry_run.sdc` and `dry_run_pdn_cfg.tcl` were folded into `config.yaml` / `grouper_chip_core.sdc` / `pdn_cfg.tcl`.
- **`ahb_stub_slave` is sized, not just present.** The three fabric slots without real peripherals wired in (QSPI and SPI Master always, SPI Slave under `DRY_RUN`) hold `hw/rtl/interconnect/ahb_stub_slave.sv`, which takes a `TARGET_GE` parameter and drives/consumes the block's real `io_ss` pad signals. Both matter for PnR: without the pad connections `io_ss` sees constants and the alternate-function paths for pads 4–14 fold away, and without `TARGET_GE` the slot costs a fixed ~64 flops regardless of what will replace it. Targets are (synthesized area of the block's existing RTL) × (a multiplier for the features still to be built); `librelane/measure/` holds synthesis-only configs for that measurement (`make measure-ge`), and `scripts/report_ge.py` converts area to gate equivalents at 1 GE = 10.976 µm².
