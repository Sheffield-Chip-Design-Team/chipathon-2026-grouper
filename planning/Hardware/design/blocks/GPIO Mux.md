...
The previous revision of `GRPR-GPIO-003` listed **UART** among the peripherals sharing muxed pins. It does not. `uart_tx`/`uart_rx` are dedicated pads routed straight out of `digital_ss` and `grouper_soc_top`, and never enter this block — the console must stay available regardless of how the mux is configured, including when firmware has misconfigured it. Only SPI Slave, SPI Master and QSPI share pads with GPIO.
...
| `GRPR-GPIO-007` | `GPIO_RO_MASK` shall mark pads read-only. A write to `GPIO_OUT` that would **change** the output value of any read-only pad shall be rejected in full — no bit of `GPIO_OUT` updates — and shall raise an error response per `GRPR-GPIO-010`. |
...
The synchroniser flops themselves are instantiated at `grouper_soc_top` (16 × `hw/rtl/common/sync.sv`, `DEPTH=2`), **not** inside this block — pad inputs are cleaned as they enter the digital domain, before they reach either this block or the serial peripherals. This block owns the `GPIO_SYNC_EN_N` register that selects the bypass. The earlier revision of this requirement placed the synchroniser inside the block; the register still lives here, so the firmware-visible behaviour is unchanged.

Consequence for DV: the current top-level testbench instantiates `digital_ss` directly, so it sits *below* the synchronisers. `GPIO_SYNC_EN_N` can only be verified end-to-end once the testbench instantiates `grouper_soc_top` — see [Open Items](#open-items).
...
| Pad | Alternate function | Direction |
|---|---|---|
| 0 | `spi_s_ss` | in |
| 1 | `spi_s_sck` | in |
| 2 | `spi_s_mosi` | in |
...
|---|---|---|
| Write `GPIO_OUT` | `\|((HWDATA ^ GPIO_OUT) & GPIO_RO_MASK & byte_enable)` | ERROR |
