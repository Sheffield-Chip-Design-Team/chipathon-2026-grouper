...
# From: https://gitlab.com/Kamcuk/kamilscripts/-/blob/master/bin/,stacklyze.sh
# https://stackoverflow.com/a/67398286
...
dbgt() {
	if [ "${DEBUG:-false}" = "true" ]; then
		echo "==> $2 <=="
		# column -t -s$'\t' -o ' | ' -N "$1" "$2"
		# yucky fix to add headers to old version of SED
		echo "$1" | sed 's/,/\t/g' > "$2.hdr"
		column -t -s$'\t' -o ' | ' "$2.hdr" "$2"
		echo
	fi
}
...
sed -nE '
	...
	# try and find subtraction from stack pointer
	# /.*\ssp\)\s*\n/b print
	/.*\(plus:SI.*\ssp\)\s*$/{
		N
		/.*\((const_int)\s-([0-9]+).*/b print
	};
